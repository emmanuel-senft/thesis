...
This study compares two conditions: \gls{sparc}, where the robot learns from the participant's selections and the \gls{woz} condition where the robot is simply controlled by the participant. As this was the first version evaluating \gls{sparc}, the time component of the interaction was artificially discretised, however, this limitation was addressed in future work (cf. Chapter~\ref{chap:tutoring}). In both conditions, actions can only be executed in predefined time windows dictated by the dynamics of the interaction (cf. Section~\ref{ssec:woz_wizarded_robot}). As some situations would require the robot not to act, a `wait' action was added to the \gls{sparc} condition and presented an active choice for the participants.
