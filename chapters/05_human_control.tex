...
%		\hspace{-20pt} 
			...
			\While{learning}{
			 	$a_{t}$ = $ arg \underset{a \in A_t}{max} Q[s_{t},a]$
...
%			\hspace{-20pt}
			\SetKwInOut{Input}{inputs}\SetKwInOut{Output}{outputs}
			\Input{
				$s_{t+1}$: current state\\
				$A_{t+1}$: actions available\\
				$Q$: Q-table\\
			}
			\Output{
				$a_{t+1}$: selected action\\
				$Q$: updated Q-table\\
			}
		 \While{learning}{
			$A'_{t+1}=[a^{1}...a^n]$, $n$ actions with high $Q[s_{t+1},a^{i}]$
			...
			\While{waiting for guidance and reward on $a_{t}$ (2 seconds)}{
				...
				\eIf{receiving guidance}{
					$a_{t+1}$ = guidance
				}{
					$a_{t+1}$ = $ arg \underset{a \in A_{t+1}}{max} Q[s_{t},a]$
				}
			}
...
Similarly to the original study, the robot indicates its intention and confusion by \emph{gazing} to objects. In this case, the head of the robot would point towards an object indicating that the robot considers the action related to this object. This gazing behaviour is simple to interpret as objects are located in space in such a way that only one can be faced by the robot at the same time. In the case of confusion, the robot would move its head between different direction indicating the $n$ objects it is considering to use in its next action.
...
This chapter extended \gls{sparc} and compared it to an other method from the \gls{iml} field. \gls{sparc} succeeded in its goal, allowing participants to teach easily and safely a policy to a robot. Finally, these encouraging results supported us in the decision to move on to 
%insights from this study have been used to guide the design of the study presented in Chapter~\ref{chap:tutoring}, 
the final study of this research (presented in Chapter~\ref{chap:tutoring}), which addressed some of the limitations of this study by exploring how a human could teach a robot to interact with children in real-world \gls{hri}.
